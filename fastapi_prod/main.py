...
@app.get(
    ...
    description=(
        "Возвращает краткую информацию о рецептах, отсортированную "
        "сначала по популярности (count_views ↓), "
        "затем по времени приготовления (↑)."
    ),
)
...
@app.get(
    ...
    description=(
        ...
        "и увеличивает счётчик просмотров на 1."
    ),
)
...
@app.post(
    ...
    description=(
        "Создаёт новый рецепт с ингредиентами и описанием. "
        "Возвращает всю информацию о созданном рецепте."
    ),
)
